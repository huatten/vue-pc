...
var webpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    //   },
    //   // necessary to consistently work with historyple chunks via CommonsChunkPlugin
    //   chunksSortMode: 'dependency'
    // }),
    /* 注释这个区域的内容 ---------------------- 结束 */
...
原先，入口文件只有一个main.js,但现在由于是多页面，因此入口页面多了，我目前就是两个：index和history，之后如果打包，就会在`dist`文件下生成两个HTML文件：`index.html`和`history.html`(可以参考一下单页面应用时，打包只会生成一个index.html,区别在这里)。

history文件下的三个文件，就是一般模式的配置，参考index的就可以，但并不完全相同。
...
new Vue({
    el:'#app'，// 这里参考history.html和history.vue的根节点id，保持三者一致
    teleplate：'<history/>',
    components:{ history }
})
```
...
这里是修改`package.json`的resolve下的vue的配置，很多人反应这样修改之后就好了，但是我按照这个方法修改之后依然报错。然后我就想到上面提到的`render`函数，因此我的修改是针对`history.js`文件的。
...
既然是多页面，肯定涉及页面之间的互相跳转，就按照我这个项目举例，从index.html文件点击a标签跳转到history.html。

我最开始写的是：

```html
 <!-- index.html -->
<a href='../history/history.html'></a>
```
